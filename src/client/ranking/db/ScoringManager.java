...
public class ScoringManager {
	...
	public ArrayList<Game> showAllGames(){
		Cursor cursor=database.rawQuery("Select "+GamesColumns.GAME_DATE_CREATED+","+GamesColumns.GAME_ID+
	...
	public void showAllGamesAllRounds(){
		Cursor cursor=database.rawQuery(
//					"select AllRound.roundId RoundId,AllRound.gameId,RD.userId,U.userName,RD.point "+
//					"from "+
//
//						"(" +
						"select  R.roundId,R.gameId, dateUpdated, row_number() over(partition by gameId order by dateUpdated desc) Rid " +
							"from "+
							Tables.GAMES+" g join "+Tables.ROUNDS+" R on G."+GamesColumns.GAME_ID+"=R."+RoundsColumns.ROUND_GAME_ID
//						")"+
//						"AllRound "+
//						"join"+
//						Tables.ROUNDS_DETAILS+" RD on R."+RoundsDetailsColumns.RDETAIL_ID+"=RD."+RoundsColumns.ROUND_ID+
//						" join "+
//						
//						Tables.USERS+" U on RD."+UsersColumns.USER_ID+"=U."+RoundsDetailsColumns.RDETAIL_USERID+
//						" where"+
//						"AllRound.Rid=1"
	...
	public void showAllRounds(Game game){
		Cursor cursor=database.rawQuery(
				"Select "+"R."+RoundsColumns.ROUND_ID+",R."+RoundsColumns.ROUND_DCREATED+",G."+RoundsDetailsColumns.RDETAIL_USERID+","+"G."+RoundsDetailsColumns.RDETAIL_POINT+
				" from "+Tables.ROUNDS +" R join "+Tables.ROUNDS_DETAILS+" G on R."+RoundsColumns.ROUND_ID +"=G."+RoundsDetailsColumns.RDETAIL_ID
				+" where R."+RoundsColumns.ROUND_GAME_ID+"="+game.getGameId()
				,null);
		cursor.moveToFirst();
		long roundid=-1;
		Round round= new Round();
		ArrayList<Round> rounds= new ArrayList<Round>();
		if(!cursor.isAfterLast()){//first iteration
